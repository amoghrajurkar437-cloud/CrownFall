...
# Multiple buttons on 1 screen
trade_button_rects = None  # ADDED: global storage for BUY/SELL rects
...
def draw_trade_menu(surface):
    ...
    # ADDED: Draw BUY/SELL buttons (centered under shop items)
    btn_w, btn_h = 150, 55
    btn_spacing = 40
    total_w = btn_w * 2 + btn_spacing
    btn_x = box_x + (box_w - total_w) // 2
    btn_y = y + 12
    buy_rect = pygame.Rect(btn_x, btn_y, btn_w, btn_h)
    sell_rect = pygame.Rect(btn_x + btn_w + btn_spacing, btn_y, btn_w, btn_h)
    # Button visuals
    pygame.draw.rect(surface, (0, 160, 0), buy_rect)  # ADDED: BUY = green
    pygame.draw.rect(surface, (255, 255, 255), buy_rect, 2)
    buy_txt = font.render("BUY", True, (255, 255, 255))
    surface.blit(buy_txt, buy_txt.get_rect(center=buy_rect.center))
    pygame.draw.rect(surface, (160, 0, 0), sell_rect)  # ADDED: SELL = red
    pygame.draw.rect(surface, (255, 255, 255), sell_rect, 2)
    sell_txt = font.render("SELL", True, (255, 255, 255))
    surface.blit(sell_txt, sell_txt.get_rect(center=sell_rect.center))
    # ADDED: store button rects on screen for click detection elsewhere
    global trade_button_rects
    trade_button_rects = {'buy': buy_rect, 'sell': sell_rect}  # ADDED
...
while running:
    ...
    # Clear ephemeral shop rects each fram
    if hasattr(screen, "_shop_item_rects"):
        del screen._shop_item_rects
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and not on_home:
            ...
            # ADDED: Left-click handling for BUY/SELL buttons when trade menu is active
            if event.button == 1 and trade_menu_active:
                # Use the rects stored by draw_trade_menu
                btns = trade_button_rects  # ADDED
                if btns:
                    # If clicked BUY
                    if btns['buy'].collidepoint(event.pos):
                        ...
                        continue
                    # If clicked SELL
                    if btns['sell'].collidepoint(event.pos):
                        Item_key, price = trade_items[trade_selection]
        ...
        # ----- Toggles & Keyboard -----
            ...
            if trade_menu_active:
                ...
                # ADDED: BUY KEY removed; now handled via mouse buttons
                # ADDED: SELL KEY removed; now handled via mouse buttons
        ...
        for rect, ax, ay in item_list:
            if player.colliderect(rect):
                if inventory[inventory_key] < inventory_limits[limit_key]:
                    ...
                    globals()[f"c_{inventory_key.replace(' ', '_')}"].add((current_room[0], current_room[1], current_room[2], ax, ay))
                    message, message_color, message_timer = msg, color, 0.75
                else:
                    message, message_color, message_timer = "Inventory full!", color, 0.75

    pickup(artifacts, "Artifacts", "Artifacts", 1, (255, 215, 0), "Picked up artifact")
    pickup(gold, "Gold", "Gold", gold_per_pickup, (255, 215, 0), "Picked up gold")
    pickup(health_potions, "Health Potions", "Health Potions", 1, (0, 255, 0), "Picked up health potion")
    pickup(speed_potions, "Speed Potions", "Speed Potions", 1, (0, 0, 255), "Picked up speed potion")
    pickup(strength_potions, "Strength Potions", "Strength Potions", 1, (255, 0, 0), "Picked up strength potion")
    ...
    if message_timer > 0:
        ...
        message = ""
    if feedback_timer > 0:
        feedback_timer -= dt/1000.0
    else:
        feedback = ""

    # Update potion timers
    if speed_potion_duration > 0:
        speed_potion_duration -= dt/1000.0
    if strength_potion_duration > 0:
        strength_potion_duration -= dt/1000.0
